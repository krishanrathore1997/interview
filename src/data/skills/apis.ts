...
{"amount": 4999, "currency": "INR"}`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why is PATCH not idempotent by default even though PUT is?',
              answer: 'PUT is defined as "replace the entire resource with this representation," so sending the same PUT twice leaves the resource in the same final state. PATCH often describes a relative change, like incrementing a counter or appending to a list, so applying it twice can produce a different result than applying it once — unless the API deliberately defines its PATCH semantics to be idempotent.',
              difficulty: 'Medium',
            },
          ],
        },
  ...
  "error": {
    ...
    "fields": { "email": "Invalid email format" }
          ...
          interviewQuestions: [
            ...
            {
              question: 'When would you return 409 instead of 422?',
              answer: '422 is for the request itself being semantically invalid, like a malformed email or a missing required field. 409 is for a valid request that conflicts with the current server state, like creating a user with an email that\'s already taken, or updating a record someone else changed first.',
              difficulty: 'Medium',
            },
          ],
        },
...
GET    /users/:id/orders`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why is POST /payments/:id/capture an acceptable exception to strict resource modeling?',
              answer: 'Not every real-world action maps cleanly onto CRUD verbs on a noun — "capture" is a business command with its own side effects and validation, not a field update. Modeling it as a sub-resource action endpoint keeps the intent explicit and auditable, clearer than forcing it into a generic PATCH that overloads meaning.',
              difficulty: 'Medium',
            },
          ],
        },
  ...
  "nextCursor": "eyJpZCI6MTAyfQ=="
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why should a cursor be treated as an opaque token instead of a raw database ID?',
              answer: 'Exposing a raw ID as the cursor leaks internal details and invites clients to construct or guess cursors themselves, which breaks if you ever change the underlying sort key or storage. Encoding or signing the cursor keeps the pagination contract stable and hides implementation details.',
              difficulty: 'Medium',
            },
          ],
        },
      ...
      concepts: [
        {
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why is revocation harder with JWTs than with server-side sessions?',
              answer: 'A server-side session can be invalidated instantly by deleting its record from the session store. A JWT is self-contained and stays valid until it expires purely by its signature — the server has no built-in way to cancel one early without maintaining a separate revocation list, which undercuts the point of being stateless.',
              difficulty: 'Hard',
            },
          ],
        },
...
Access-Control-Allow-Credentials: true`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why do browsers specifically disallow Access-Control-Allow-Origin: * combined with credentials?',
              answer: 'If a wildcard origin were allowed alongside credentialed requests (cookies, auth headers), any website on the internet could make an authenticated request to your API and read the response, defeating the point of same-origin protections. Browsers require an explicit origin whenever credentials are involved.',
              difficulty: 'Hard',
            },
          ],
        },
...
HTTP/1.1 304 Not Modified`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'What\'s the difference between a strong validator and a weak validator?',
              answer: 'A strong validator (a standard ETag) guarantees byte-for-byte identical content when it matches, safe for range requests and exact comparisons. A weak validator (prefixed W/) only guarantees semantic equivalence, which is looser but cheaper to compute for content that changes formatting without changing meaning.',
              difficulty: 'Hard',
            },
          ],
        },
...
Retry-After: 30`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why can IP-based rate limiting unfairly punish legitimate users?',
              answer: 'Many users can share one public IP behind NAT, a corporate network, or a mobile carrier\'s gateway, so one abusive user or a buggy client can exhaust the shared limit and get everyone behind that IP blocked. Limiting by an authenticated identity, like a user ID or API key, avoids that collateral damage when it\'s available.',
              difficulty: 'Medium',
            },
          ],
        },
...
4) Return 200 quickly`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why verify a webhook signature before parsing the payload, not after?',
              answer: 'Parsing, especially with a permissive or vulnerable parser, runs directly on the payload, so processing an unverified payload first means potentially attacker-controlled data runs through your code before you\'ve confirmed it\'s legitimate. Verifying the HMAC signature first ensures nothing downstream ever touches a payload that didn\'t come from the real provider.',
              difficulty: 'Medium',
            },
          ],
        },
...
Never retry: 4xx validation`,
          ...
          interviewQuestions: [
            ...
            {
              question: 'Why does jitter matter in a backoff strategy, not just the exponential delay itself?',
              answer: 'If every failed client retries at exactly the same fixed intervals, they all hit the server again at the same moment, creating synchronized waves of load, sometimes worse than no backoff at all. Random jitter spreads those retries out over time, smoothing the load instead of creating a thundering herd.',
              difficulty: 'Hard',
            },
          ],
        },
